...
model = MLP(input_dim=2, hidden_layers=hidden_layers, output_dim=1, activation=activation)
...
def plot_decision_boundary(model, X, y):
    ...
    # Fond : probabilité de classe 1
    contour = ax.contourf(xx, yy, probs, levels=50, cmap="RdYlBu_r", alpha=0.8)
    plt.colorbar(contour, ax=ax, label="P(classe 1)")

    # Frontière de décision (prob = 0.5)
