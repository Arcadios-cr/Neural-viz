...
with st.expander("À quoi sert cet outil ?"):
    st.markdown(
        ...
        "réseau de graphe bat (ou non) un MLP. Réponse, démontrée dans l'outil : l'info "
        "du voisinage n'aide que si elle est **absente des features** (datasets **SBM**, "
        "**Density découplée**) ET **lisible par l'agrégation** — la moyenne normalisée "
        "efface le comptage (donc la densité), la **somme** (GIN) sait compter, et la "
        "**géométrie des arêtes** (longueur, verticalité) peut aussi être donnée en "
        "features. Et empiler trop de couches finit par tout **lisser** (sur-lissage).\n\n"
...
dataset_name = st.sidebar.selectbox(
    ...
    help=(
        ...
        "Density — bleu très dense / rouge très clairsemé qui se chevauchent : "
        ...
        "densité ?)\n"
        "Density découplée — micro-amas bleus denses éparpillés PARTOUT + rouge "
        "clairsemé uniforme : la position ne dit rien (MLP = baseline), la densité "
        "locale dit tout — le juge de paix : seul un graphe par RAYON + l'agrégation "
        "SOMME (GIN) la lisent\n"
        "Structure — bleu en colonnes / rouge en lignes (bleu à gauche, rouge à "
        "droite) avec chevauchement : dans la bande commune, l'indice est "
        "l'ARRANGEMENT local (vertical vs horizontal) — à tester en mode « Réseau "
        "de graphe » en comparant les agrégations\n"
        ...
        "SBM — graphe communautaire FOURNI + features 2D faibles : le cas où le "
